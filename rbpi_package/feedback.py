...
class FeedbackController:
    ...
    def indicate_armed(self):
        ...
        if self.current_state == "armed" and self.led_thread and self.led_thread.is_alive():
            return
    ...
    def indicate_disarmed(self):
        ...
        if self.current_state == "disarmed" and self.led_thread and self.led_thread.is_alive():
            return
    ...
    def indicate_stuck_alarm(self):
        ...
        if self.current_state == "stuck_alarm" and self.led_thread and self.led_thread.is_alive():
            return
        self.current_state = "stuck_alarm"
        print("[Feedback] Stuck Alarm -> Rapid Red Flash & Siren")
        if self.led_thread and self.led_thread.is_alive():
            self._stop_event.set()
            self.led_thread.join()
